...
export const DOWNLOAD_BASE_URL = "https://www.googleapis.com/download/storage/v1/b/chromium-browser-snapshots/o/Win_x64%2F";
    ...
    if (fs.existsSync(dir)) {
        fs.readdirSync(dir).forEach(function(entry) {
            ...
            if (fs.lstatSync(entryPath).isDirectory()) {
                if (rimrafExcept(entryPath, except))
                    hasException = true;
            } else {
...
async function install(version: string) {
    ...
    try {
        ...
        const promise = got(url, {encoding: null}) as any;
        let sizeInitialized = false;
        (promise as any).on("downloadProgress", (p: got.Progress) => {
            if(sizeInitialized) {
        ...
        if(fs.existsSync(APP_FOLDER))
            rimrafExcept(APP_FOLDER, [path.join(APP_FOLDER, 'User Data')]);
        ...
        for(const entry of zip.getEntries()) {
            ...
            if (!entry.isDirectory)
                fs.writeFileSync(path, entry.getData())
